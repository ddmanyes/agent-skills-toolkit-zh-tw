...
flowchart TD
    ...
    Context --> Need{"仍有實質設計決策？"}
    Need -->|沒有，範圍清楚| Scope{"需要多階段書面計畫？"}
    Need -->|有| Brain["sp-brainstorming<br/>釐清未解的意圖、限制與取捨"]
    Brain --> Gap{"還有資料缺口？"}
    Gap -->|外部可查事實| Research["research<br/>查一手來源"]
    Gap -->|只有利害關係人知道| Questions["to-questionnaire<br/>整理必要問題"]
    Research --> Brain
    Questions --> Brain
    Gap -->|需要領域語言／ADR| Domain["domain-modeling"]
    Gap -->|資訊已足夠| Scope
    Domain --> Scope
    Scope -->|是| Plan["sp-writing-plans<br/>跨輪任務加 planning-with-files"]
    Scope -->|否| Direct["直接執行已授權工作<br/>按影響驗證"]
    ...
    class Start,Plan,Direct startEnd;
    class Gap,Need,Scope gate;
```
...
需要完整設計與計畫的任務按三張圖銜接；範圍清楚的小任務可直接執行，再進入相應驗證。每個決策出口都必須符合線上的條件。不因任務「看起來很大」就自動取得自主執行、平行代理或 Git 發佈權限。`ask-matt`、教學支援、定期架構深化與 Agent 文件維護則按下方表格的條件旁路啟用。
...
| 1. 建立骨架 | `project-architect` | Python／uv 綠地專案 | uv、Ruff、實際客戶端的 Agent 導航與專案基線 |
| 1. 理解既有專案 | `sp-learn-codebase` | 進入陌生或大型既有程式庫 | 架構、依賴與關鍵元件地圖 |
| 2. 釐清設計 | `sp-brainstorming` | 有尚未解決、會改變實作的需求或設計決策 | 明確的取捨與驗收；沿用已給授權，不重複批准 |
...
| 3. 寫計畫 | `sp-writing-plans` | 需求明確且工作需要多階段協調 | 可執行、可驗證的多步驟計畫 |
...
| 4. 測試先行 | `tdd` | 使用者要求 test-first、red-green-refactor，或明確選用 TDD 的整合測試 | 由失敗測試帶動的實作 |
...
3. **完成宣告必須有證據**：按改動影響選擇測試、lint、build、實際輸出或外部寫入核對；不為每次低影響文字修改機械增加測試。
