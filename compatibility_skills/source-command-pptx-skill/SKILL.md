---
name: source-command-pptx-skill
description: Use when the user explicitly invokes the legacy pptx-SKILL command; preserve that invocation through its maintained workflow.
---

# 舊命令 pptx-SKILL

僅在使用者明確要求 `pptx-SKILL` 或 `$source-command-pptx-skill` 時使用；不自行攔截一般任務。
